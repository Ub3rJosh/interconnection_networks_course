...
#define PSDELAY  if (YS__Cycles > 0) YS__PSDelay();
...
typedef struct YS__Proc       PROCESS;
...
extern PROCESS  *YS__ActProc;         /* Pointer to the currently active process      */
...
extern POOL     YS__CxtPool;          /* Pool of CONTEXT descriptors                  */
extern POOL     YS__StkPool;          /* Pool of stacks of the default stack size     */
extern POOL     YS__ProcPool;         /* Pool of PROCESS descriptors                  */
...
static struct YS__Qelem {       /* Used only with state variables and pools           */
...
static struct YS__Pool {        /* Used to minimize the use of malloc                 */
   char name[32];               /* User defined name                                  */
   char *head;                  /* Pointer to the first element of the queue          */
   char *tail;                  /* Pointer to the last element of the queue           */
   char *middle;                /* Pointer to the division between idle and active    */
...
void YS__PoolInit();            /* Initialize a pool                                  */
char *YS__PoolGetObj();         /* Get an object from a pool                          */
void YS__PoolReturnObj();       /* Return an object to its pool                       */
void YS__PoolReset();           /* Deallocate all objects in a pool                   */
...
static struct YS__Qe {          /* Queue element; base of most other objects          */
   char *pnxt;                  /* Next pointer for Pools                             */
...
static struct YS__Queue {  /* Basic queue of QEs; derived from QE                     */
                           /* QE fields */
   char    *pnxt;               /* Next pointer for Pools                             */
   QE      *next;               /* Pointer to the element after this one              */
...
static struct YS__SyncQue {/* Synchronization Queue; base of sema, flg, cond, barr    */
                           /* QE fields */
   char    *pnxt;               /* Next pointer for Pools                             */
   QE      *next;               /* Pointer to the element after this one              */
   int     type;                /* Identifies the type of queue element               */
...
static struct YS__StatQe { /* Statistical queue element; base of activities & msgs    */
                           /* QE fields */
   char       *pnxt;            /* Next pointer for Pools                             */
   QE         *next;            /* Pointer to the element after this one              */
   int        type;             /* PROCTYPE or EVTYPE                                 */
...
static struct YS__Act {    /* Activity; base of processes and events                  */
                           /* QE fields */
   char       *pnxt;            /* Next pointer for Pools                             */
   QE         *next;            /* Pointer to the element after this one              */
   int        type;             /* PROCTYPE or EVTYPE                                 */
   ...
   PROCESS    *parent;          /* Pointer to the process that forked this ACTIVITY   */
...
static struct YS__Proc {
   ...
   PROCESSOR  *pp;              /* This process is attached to the processor *pp      */
                           /* PROCESS fields */
   int        *cxt;             /* The context for this process                       */
   PROCESS    *prev;            /* Pointer to process that transferred here           */
   int        children;         /* Number of active child processes                   */
   QUEUE      *MsgList;         /* Queue of message sent to this process              */
   int        waitmsgtype;      /* Type of message this process is waiting for        */
   PROCESS    *waitmsgsrc;      /* Sender this process is waiting for                 */
};

/****************************** PROCESS Operations ************************************/

int      YS__Transfer();        /* Transfers control to another process               */
int      YS__Suspend();         /* Suspends the current process                       */
...
static struct YS__Mess {
   ...
   double    enterque;
                           /* MESSAGE fields */
   PROCESS   *receiver;         /* Pointer to receiving process, processor, or port   */
...
void   YS__ContextInit();
void   YS__ContextFree();
...
void   SetSysRand();           /* Specify the random number generator to use          */
