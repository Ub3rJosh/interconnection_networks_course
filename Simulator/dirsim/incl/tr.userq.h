...
#define TRACE_SEMAPHORE_p1  \
   if (TraceLevel >= MAXDBLEVEL-2)  { \
      if (YS__ActProc != NULL)  \
         sprintf(YS__prbpkt, \
            "    Process %s[%d] continues past semaphore %s[%d], new value = %d\n",\
                 YS__ActProc->name,YS__ActId(YS__ActProc), \
                 sptr->name,YS__QeId(sptr),(sptr->val)-1); \
      YS__SendPrbPkt(TEXTPROBE,sptr->name,YS__prbpkt); \
   }
...
#define TRACE_SEMAPHORE_p2  \
   if (TraceLevel >= MAXDBLEVEL-2) { \
      if (YS__ActProc != NULL)  \
         sprintf(YS__prbpkt, \
            "    Process %s[%d] waiting at semaphore %s[%d], value = %d\n", \
                 YS__ActProc->name,YS__ActId(YS__ActProc), \
...
#define TRACE_SEMAPHORE_v1p  \
   if (TraceLevel >= MAXDBLEVEL-2)  { \
      ...
      if (YS__ActProc != NULL)  { \
         sprintf(YS__prbpkt,"    Process %s[%d] signalling semaphore %s[%d]\n", \
                 YS__ActProc->name,YS__ActId(YS__ActProc),sptr->name,YS__QeId(sptr)); \
         sprintf(YS__prbpkt+strlen(YS__prbpkt), \
            "    - No processes or events waiting; new semaphore value = %d\n", \
                 sptr->val); \
      }  \
...
#define TRACE_SEMAPHORE_s1p  \
   if (TraceLevel >= MAXDBLEVEL-2)  { \
      ...
      if (YS__ActProc != NULL)  { \
         sprintf(YS__prbpkt,"    Process %s[%d] setting semaphore %s[%d]\n", \
                 YS__ActProc->name,YS__ActId(YS__ActProc),sptr->name,YS__QeId(sptr)); \
         sprintf(YS__prbpkt+strlen(YS__prbpkt), \
            "    - No processes or events waiting; new semaphore value = %d\n", \
                 sptr->val); \
...
#define TRACE_BARRIER_sync1  \
   if (TraceLevel >= MAXDBLEVEL-2) { \
      sprintf(YS__prbpkt,"    Process %s[%d] synchronizing on barrier %s[%d]\n",  \
              YS__ActProc->name,YS__ActId(YS__ActProc),bptr->name,YS__QeId(bptr)); \
      YS__SendPrbPkt(TEXTPROBE,bptr->name,YS__prbpkt); \
   }
...
#define TRACE_FLAG_wait1  \
   if (TraceLevel >= MAXDBLEVEL-2) {  \
      sprintf(YS__prbpkt,"    Process %s[%d] waiting at flag %s[%d]\n",  \
              YS__ActProc->name,YS__ActId(YS__ActProc),fptr->name,YS__QeId(fptr));  \
...
#define TRACE_FLAG_set1p  \
   if (TraceLevel >= MAXDBLEVEL-2) {  \
      if (YS__ActEvnt != NULL)  \
         ...
         sprintf(YS__prbpkt,"    Process %s[%d] setting flag %s[%d]\n",  \
                 YS__ActProc->name,YS__ActId(YS__ActProc),fptr->name,YS__ActId(fptr)); \
      else  \
...
#define TRACE_FLAG_release1p  \
   if (TraceLevel >= MAXDBLEVEL-2) {  \
      if (YS__ActEvnt != NULL)  \
         ...
         sprintf(YS__prbpkt, \
            "    Process %s[%d] releasing all activities waiting at flag %s[%d]\n", \
                 YS__ActProc->name,YS__ActId(YS__ActProc),fptr->name,YS__QeId(fptr)); \
      else  \
...
#define TRACE_CONDITION_wait1  \
   if (TraceLevel >= MAXDBLEVEL-2) {  \
      sprintf(YS__prbpkt,"    Process %s[%d] waiting at condition %s[%d]\n", \
              YS__ActProc->name,YS__ActId(YS__ActProc),cptr->name,YS__QeId(cptr));  \
...
#define TRACE_RESOURCE_use1  \
   if (TraceLevel >= MAXDBLEVEL-2) { \
      sprintf(YS__prbpkt, \
         "    Process %s[%d] requests ",YS__ActProc->name,YS__ActId(YS__ActProc)); \
      sprintf(YS__prbpkt+strlen(YS__prbpkt), \
