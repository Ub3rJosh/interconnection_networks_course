...
#define ORDBUF					5000
#define INVCBUF					5100
#define	OUTVCBUF				5200

#define ORDDEMUX				6000
#define DEMUX_ROUTER			6100
#define DEMUX_SWITCH			6200
#define DEMUX_RECON				6300

#define ORDMUX					7000
#define MUX_SWITCH				7100
...
typedef struct YS__LinkMsg 		LINKMSG;
typedef struct YS__ControlMsg	CTRLMSG;
...
typedef int  MUX;
typedef int  DEMUX;
typedef int  DUPLEXMOD;
typedef int  OPORT;
typedef int  IPORT;
typedef int  PACKET;
typedef int  (*rtfunc)();  /* Pointer to a routing function                           */

typedef int OCOUPLER;
typedef int OSPLITTER;
typedef int OWAVEGUIDE;
typedef int OFIBER;
typedef int ODEMUX;
typedef int OTOKEN;
typedef int OAWG;
typedef int USERQUEUE;
...
struct YS__PktData
{
   ...
   int		ibufs;
   int		link;
   int	   switching;		/* Indicates the switching mode */
   int	   ckt;			/* Indicates probe/ack/data flits */
   int	   tag;
  	double tokenblktime;
  	int iport_awg;
  	int lambda;
...
OCOUPLER	*NewOCoupler();		  /* Creates and returns a pointer to a new coupler */
OFIBER		*NewOFiber();			/* Creates and returns a pointer to a new coupler	*/
OWAVEGUIDE	*NewOWaveguide();			/* Creates and returns a pointer to a new coupler	*/
ODEMUX		*NewODemux();			/* Creates and returns a pointer to a new coupler	*/
OSPLITTER	*NewOSplitter();
OTOKEN		*NewOToken();
OAWG		*NewOAwg();
USERQUEUE	*NewUserQueue();
CTRLMSG		*NewControlMsg();

void OpticalNetworkConnect();
double OpticalPacketSend();
void TokenStart();
void TokenEnd();

void BufConnectVC();
void BufReverseConnect();
int  CheckFreeMux();
int  CheckFreeBuf();
int AddQ();
int CheckQ();
...
double TimeQ();
double UpdateQstat();
SEMAPHORE *UserQSemaphore();
void UpdateChannelQ();
int ReturnChannelQ();
void UpdateOnOff();
int ReturnOnOff();
int ReturnQID();
CTRLMSG *ControlGetData();
void MuxConnectVC();
void UpdateQSeq();
int ReturnQSeq();
void UpdateQcount();
