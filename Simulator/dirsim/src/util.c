...
POOL    YS__CxtPool;                     /* Pool of CONTEXT descriptors               */
POOL    YS__StkPool;                     /* Pool of stacks of default stack size      */
POOL    YS__ProcPool;                    /* Pool of PROCESS descriptors               */
...
PROCESS *YS__ActProc;                    /* Pointer to the currently active process   */
...
int     YS__PacketSize = DEFAULT_PKT_SZ; /* Packet size, can be changed by user       */
...
double  YS__OSDelays[NUMDELAYS][3];      /* Table of operating system delays          */
...
****************************************************************************************
**                                                                                    **
**     These routines provide an interface to the debugging tools viewsim and         **
**     dbsim.  They first test to see if one of these tools is active.  If not        **
**     text is sent to stdout, otherwise it goes to the tool.                         **
**                                                                                    **
****************************************************************************************
\**************************************************************************************/
...
char *name;                            /* Name of the object generating the packet    */
char *data;                            /* Packet data                                 */
{
   if (YS__interactive) {              /* Either viewsim or dbsim is active           */
      char packet_buf[1024];
      int length, i;

      /* Build a trace packet consisting of 4 fields:                                 */
      /*     type - an integer indicating the type of the probe                       */
      /*     length - number of bytes in the name field and the following data        */
      /*     name - a string tag indicating which object generated the trace data     */
      /*     data - the trace data itself                                             */

      bzero(packet_buf, TR_NAME_LENGTH+1);/* Clear the name field                     */
      if (name)                           /* Copy the object name (tag) to the buffer */
	for (i=0; (name[i]) && (i < TR_NAME_LENGTH); i++)
	  packet_buf[i] = name[i];
      bcopy(data, &packet_buf[TR_NAME_LENGTH+1], strlen(data)+1); /* copy the data    */
      length = TR_NAME_LENGTH + 2 + strlen(data);

      write(SIMIO, &type, 1);                    /* Send the packet type              */
      write(SIMIO, &length, sizeof(length));     /* Send the length field             */
      write(SIMIO, packet_buf, length);          /* Send the data                     */
   }
   else                        /* Debugging tools not active; send data to stdout     */
      if (type==TEXTPROBE)     /* The only types of probes that work in this case     */
         fprintf(stdout,data); /* are TEXT PROBES                                     */
}

/**************************************************************************************/

void TracePrint(va_alist)     /* Sends user generated messages to text probes         */
va_dcl
{
    va_list var;
    char *fmt;
...
****************************************************************************************
**                                                                                    **
**     These routines realize the delays due to executed cycles as determined         **
**     by profiler inserted routines and delays for operating system operations.      **
**                                                                                    **
****************************************************************************************
\**************************************************************************************/

void YS__PSDelay()       /* Called by parcsim to delay for profiler cycles            */
{
   double delaytime;

   if ( YS__ActProc != NULL &&             /* A process is active                  */
        YS__ActProc->type == USRPRTYPE)    /* and the process is a user process        */
   {                                      /* i.e, created by NewUserProcess           */
      delaytime = ((double)YS__Cycles)*YS__CycleTime;
      YS__Cycles = 0;
      ProcessDelay(delaytime);
   }
}

/**************************************************************************************/

void SetCycleTime(ctime)                   /* Sets the cycle time                     */
double ctime;                              /* New cycle time                          */
{
   YS__CycleTime = ctime;
}
...
void SetOSDelay(type,d0,d1,d2)   /* Set OS delays                                     */
int type;                        /* Delay type to be set                              */
double d0, d1, d2;               /* Delay values (some delay types may not use all 3) */
{                                /* Delays are of the form d0 + x*d1 + y*d2           */
   YS__OSDelays[type][0] = d0;
   YS__OSDelays[type][1] = d1;
   YS__OSDelays[type][2] = d2;
}
