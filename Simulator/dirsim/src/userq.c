...
****************************************************************************************
**                                                                                    **
**         These operations implement the standard semaphore queue.  Since            **
**         this code is for a uniprocessor, there is no need to implement             **
**         the operations as indivisible.                                             **
**                                                                                    **
****************************************************************************************
\**************************************************************************************/
...
int SemaphoreInit(sptr,i) /* If queue is empty its value is set to i.  Returns the    */
                          /* size of the semaphore's queue                            */
SEMAPHORE *sptr;          /* Pointer to the semaphore                                 */
int i;                    /* New value for the semaphore                              */
{
   int num;
   ...
   num = sptr->size;
   TRACE_SEMAPHORE_init;                /* Initializing semaphore; new value = ...    */
   if (sptr->size == 0) {               /* No change if queue is not empty            */
      sptr->val = i;
      QueueResetStats(sptr);
   }
   return num;
}
...
SEMAPHORE *sptr;         /* Pointer to the semaphore                                  */
{
   ...
   if (sptr->val > 0)  {                           /* Semapore not busy               */
      TRACE_SEMAPHORE_p1;                          /* Process continues past sem      */
      sptr->val--;
   }
   else  {                                         /* Semaphore busy                  */
      TRACE_SEMAPHORE_p2;                          /* Process waiting at semaphore    */
      YS__ActProc->status = WAIT_SEM;              /* Process changes state           */
      if (YS__ActProc->statptr)                    /* Collect process statistics      */
         StatrecUpdate(YS__ActProc->statptr,(double)WAIT_SEM,YS__Simtime);
      YS__QueuePutTail(sptr,YS__ActProc);          /* Enter the semaphore's queue     */
      YS__Suspend();                               /* and go to sleep                 */
}  }
...
BARRIER *bptr;                /* Pointer to the barrier                               */
{
   ...
   TRACE_BARRIER_sync1;                    /* Process synchronizing on barrier        */
   if (YS__ActProc->type != PROCTYPE
       && YS__ActProc->type != OSPRTYPE
       && YS__ActProc->type != USRPRTYPE
      ) YS__errmsg("Only processes can wait at barriers");

   bptr->cnt--;
   if (bptr->cnt > 0)  {                   /* Not all processes have arrived yet      */
      TRACE_BARRIER_sync2;                 /* ... processes waiting at barrier        */
      YS__ActProc->status = WAIT_BAR;      /* Process status changes                  */
      if (YS__ActProc->statptr)            /* Collect process statistics              */
         StatrecUpdate(YS__ActProc->statptr,(double)WAIT_BAR,YS__Simtime);
      YS__QueuePutTail(bptr,YS__ActProc);  /* Enter Barrier's queue                   */
      YS__Suspend();                       /* and go to sleep                         */
   }
   else  {                                 /* Last process has now arrived            */
      TRACE_BARRIER_sync3;                 /* Releasing all waiting processes         */
      while ((procptr = (PROCESS*)YS__QueueGetHead(bptr)) != NULL) {
         TRACE_BARRIER_sync4;              /* Releasing process ...                   */
         YS__RdyListAppend(procptr);       /* Adding released process to ready list   */
      }
      bptr->cnt = bptr->initcnt;           /* Reset barrier                           */
   }
}
...
FLAG *fptr;                    /* Pointer to the flag                                 */
{
   ...
   TRACE_FLAG_wait1;                       /* Process waiting at flag ...             */
   if (fptr->state == CLEARED)  {
      TRACE_FLAG_wait2;
      YS__ActProc->status = WAIT_FLG;         /* The processes status changes         */
      if (YS__ActProc->statptr)               /* Collect process statistics           */
         StatrecUpdate(YS__ActProc->statptr,(double)WAIT_FLG,YS__Simtime);
      YS__QueuePutTail(fptr,YS__ActProc);     /* Enter the flag's queue and           */
...
CONDITION *NewCondition(va_alist)  /*Creates and returns a pointer to a new condition */
{
   cond condop;             /*Defining function                                       */
   char *cname;             /*User defined name                                       */
   STVAR *svptr;
   CONDITION *condptr;
   QELEM *qeptr;
   va_list args;
   ...
   cname = va_arg(args,char *);   /*The first argument is the condition's name        */
   condop = va_arg(args,cond);    /*The second argument is a pointer to an expression */

   condptr = (CONDITION*)YS__PoolGetObj(&YS__CondPool);
   condptr->id = YS__idctr++;
   strncpy(condptr->name,cname,31);
   condptr->name[31] = '\0';
   condptr->next = NULL;
   condptr->head = NULL;
   condptr->tail = NULL;
   condptr->size = 0;
   condptr->type = SYNCQTYPE;
   condptr->lengthstat = NULL;
   condptr->timestat = NULL;
   TRACE_CONDITION_new;                               /*Creating new condition       */
   condptr->body = condop;
   while ((svptr = va_arg(args,STVAR *)) != NULL)  {   /*Next args are stvar ptrs     */
      qeptr = (QELEM*)YS__PoolGetObj(&YS__QelemPool);  /*Construct list of stvars     */
      qeptr->optr = (char*)condptr;
      qeptr->next = svptr->CondList;
      svptr->CondList = qeptr;
   }
   condptr->argptr = va_arg(args,char *); /* Argument passed tocondition's expression */
   condptr->argsize = va_arg(args,int);   /* Size of argument                         */
   va_end(args);

   condptr->state = condptr->body(condptr->argptr,condptr->argsize); /*Evaluate cond */
   return condptr;
}

/**************************************************************************************/

void ConditionWait(cptr)     /* Waits until a condition holds                         */
CONDITION *cptr;             /* Pointer to the condition                              */
{
   PSDELAY;

   TRACE_CONDITION_wait1;                 /* Process waiting at conditon ...          */
   if (cptr->state == FALSE) {
      TRACE_CONDITION_wait2;              /* Condition false, process suspends        */
      YS__ActProc->status = WAIT_CON;     /* Process status changes                   */
      if (YS__ActProc->statptr)           /* Collect process statistics               */
         StatrecUpdate(YS__ActProc->statptr,(double)WAIT_CON,YS__Simtime);
      YS__QueuePutTail(cptr,YS__ActProc); /* Enter condition's queue                  */
      YS__Suspend();                      /* and go to sleep                          */
   }
   else {
      TRACE_CONDITION_wait3;              /* Condition true, process continues        */
   }
}

/**************************************************************************************/

int ConditionState(cptr)     /* Returns the state of a condition                      */
CONDITION *cptr;             /* Pointer to the condition                              */
{
   PSDELAY;
...
int ConditionWaiting(cptr)  /* Returns the # of activities in the queue               */
CONDITION *cptr;            /* Pointer to the condition                               */
{
   PSDELAY;
...
void ConditionSetArg(cptr,aptr,asize)  /* Set the argument passed to condition's exp. */
CONDITION *cptr;                       /* Pointer to the condition                    */
char *aptr;                            /* Pointer to the argument                     */
int asize;                             /* Argument size                               */
{
   PSDELAY;
...
double   timeinc;                   /* Amount of service requested                      */
{
   ...
   if (YS__ActProc == NULL)
      YS__errmsg("ResourceUse() must be called from a processes's  body");

   YS__ActProc->time = timeinc;     /* Amount requested service                         */
   YS__ActProc->rscptr = rptr;      /* The activity knows which resource it is in       */
   YS__RscInsert(rptr,YS__ActProc); /* This is where the work is done; see below      */
   YS__Suspend();                   /* The process sleeps until service received      */
}
