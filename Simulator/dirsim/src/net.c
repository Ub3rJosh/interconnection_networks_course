...
int	   dtype;						   /* Regular/LookAhead Router					  */
{
   ...
   demux->route = (int*)malloc(VC*sizeof(int));
   for (j = 0; j < VC; j++) {
	   demux->route[j] = -1;
   }
...
BUFFER *buf;                             /* Pointer to the buffer                     */
{
   ...
   /* Head event will wake it up head's sleeping tail after returning from HeadGet    */
	...
	/*if( (buf->id == 149) || (buf->id == 171) )
		printf("%d Getting out %d %d %g\n", buf->id, pkt->data.seqno,
									pkt->data.pkttype, YS__Simtime);*/
...
PACKET *pkt;                          /* Pointer to the packet                        */
{
   ...
   YS__DynamicBuffer(buf);
   /*YS__StaticBuffer(buf);*/

	if( buf->buftype == INPUT_BUFFER ) {
		pkt->data.powers = pkt->data.powers + BUFFER_POWER;
		pkt->data.ibufs = pkt->data.ibufs + 1;
	}
	else {
		pkt->data.powers = pkt->data.powers + CROSSBAR_POWER;
		pkt->data.xbar = pkt->data.xbar + 1;
	}
...
BUFFER *buf;                             /* Pointer to the buffer                     */
{
   ...
   /*YS__CreditFlowControl(buf, pkt);*/
   YS__IBCreditFlowControl(buf, pkt);

   /*YS__VirtualChannelRelease(buf, pkt);*/
...
int	   tag;
{
   ...
   if (port->qfree <= 0) {              /* Port full, not ready for a new packet      */
      ...
      pkt->data.intercpu = tag;

	  /*printf("Inter %d Tag %d\n", pkt->data.intercpu, tag);
	  set = 0;
      newdemux = (DEMUX*)port->nextmodule;
      for( k = 0; k < VC; k++ ) {
		ibuf = (BUFFER*)*((newdemux->nextmodule)+k);
		if( ibuf->output_seqno != -1 ) {
			pkt->data.vcindex = k;
			set = 1;
			break;
		}
...
static void YS__HeadEvent()
{
	...
	/*printf("src %d dest %d module %d seqno %d type %d size %d\n", pkt->data.srccpu, pkt->data.destcpu,
		pkt->module->id, pkt->data.seqno, pkt->data.pkttype, pkt->data.packetsize);*/
	...
	switch (EventGetState()) {
				...
				switch(CURRENT_ROUTE) {
					...
					case WF_MIN_AD:
						if( demux->demuxtype == LOOKAHEAD_DEMUX ) {
							if( pkt->data.pkttype == 0 ) {
								pkt->data.route = (demux->router)(&(pkt->data.srccpu),&(pkt->data.destcpu),demux->id);
								demux->route[pkt->data.vcindex] = pkt->data.route;
							}
							else {
								pkt->data.route = demux->route[pkt->data.vcindex];
							}
						}
						break;
					...
					case VALIANT:
						...
						if( pkt->data.routeA != 1 ) {
							pkt->data.route = (demux->router)(&(pkt->data.srccpu),&(pkt->data.intercpu),demux->id);
							if( pkt->data.route == DEGREE ) {
								pkt->data.route = (demux->router)(&(pkt->data.intercpu),&(pkt->data.destcpu),demux->id);
								pkt->data.routeA = 1;
							}
						}
						else {
					...
					default:
						YS__errmsg("Unknown Routing Approch\n");
				}
		...
		case Route_Mux: {
			...
			/*pkt->data.route = (demux_trace->router)(&(pkt->data.srccpu),&(pkt->data.destcpu),demux_trace->id);*/
			pkt->data.rcstage++;
		...
		case Virtual_Mux: {
			...
			/* Checking here to see if the input VC has been allocated already to some other packet */
			newbuf = (BUFFER*)pkt->carrybuf;
			if( newbuf->output_seqno != -1 ) {
				obuf = (BUFFER*)newbuf->outputbuf;
				EventReschedSema(obuf->bufva_sema, Virtual_Mux);
				return;
			}

			pkt->data.vastage++;
			if( (CURRENT_ROUTE == DOR_XY) || (CURRENT_ROUTE == DOR_YX) || (CURRENT_ROUTE == WF_MIN_AD) ) {
				kstart = 0;
				kend = VC;
			}

			if( (CURRENT_ROUTE == ROMM) || (CURRENT_ROUTE == VALIANT) ) {
				if( pkt->data.routeA == 0 ) {
					kstart = 0;
					kend = VC/2;
				}
				else {
			...
			do {
				if( (buf_trace->vc_data.input_vc[k] == -1) ) {
					...
					buf_trace->vc_data.input_port[k] = ((demux_trace->id)%(DEGREE+1));
					*((buf_trace->vc_data.inputbuf)+k) = pkt->carrybuf;
					...
					/*if( pkt->data.srccpu == 1 ) {*/
					/*if( (buf_trace->id == 149) || (buf_trace->id == 171) )
					printf("Allocating VC %d pkt %d obuf %d credit %d time %g\n",
			...
			/*if( (pkt->data.srccpu == 1) || (pkt->data.srccpu == 1) )
		...
		case Switch_Mux: {
			...
			if (SemaphoreValue(mux->arbsema) > 0) {
				...
				/*printf("In SMux %d at time %g pkt %d\n", mux->id, YS__Simtime,
				pkt->data.pkttype );*/
		...
		case Head_Move: {
			...
			if( buf->type == BUFFERTYPE ) {
				if( buf->buftype == OUTPUT_BUFFER ) {
					if( SemaphoreValue(buf->bufcc_sema) <= 0 ) {
						EventReschedSema(buf->bufcc_sema, Before_Head_Move);
						return;
					}
				}
			}
...
/**************************************************************************************/
void static YS__CreditFlowControl(buf, pkt)
BUFFER *buf;
PACKET *pkt;
{
	...
	if( buf->buftype == OUTPUT_BUFFER ) {
		...
		/* Credit Implementation here */
		/* Verify that the correct input buffer is being traced */
		/* Begin the Trace back to the previous router */
		/* Get the output buffer of the previous router and increment */
		/* Check if the packet has not released the VC, if it hasn't increment the credit and
			release the semaphore to allow flit traversal */
		do {
			if( buf->vc_data.input_seqno[k] == pkt->data.seqno ) {
				newbuf = (BUFFER*)*((buf->vc_data.inputbuf)+k);

				if( newbuf->output_seqno != pkt->data.seqno ) {
					printf("op buf %d id %d ip buf %d id %d Incorrect seqnos %d %d type %d\n",
						buf, buf->id, newbuf, newbuf->id, newbuf->output_seqno, pkt->data.seqno, pkt->data.pkttype);
					YS__errmsg("Incorrect seqnos\n");
				}

				newdemux = (DEMUX*)newbuf->prevmodule;
				if( newdemux->prevmodule == NULL ) {
					if( newbuf->output_alloc == -1 )
						newbuf->output_alloc = 1;
					break;
				}

				prev_vcindex = buf->vc_data.input_vc[k];
				...
				/*printf("op buf %d Vcindex %d seqnos %d type %d\n",
						output_prevbuf->id, prev_vcindex, pkt->data.seqno, pkt->data.pkttype);*/
				...
				if( output_prevbuf->vc_data.input_seqno[prev_vcindex] != -1 ) {
					...
					if( input_prevbuf->output_seqno != pkt->data.seqno ) {
						input_prevbuf->output_alloc = -1;
					}
					else {
						if( input_prevbuf->output_alloc == -1 ) {
							input_prevbuf->output_alloc = 1;
						}
					}
...
/**************************************************************************************/
void static YS__VirtualChannelRelease(buf, pkt)
BUFFER *buf;
PACKET *pkt;
{
	...
	/* Here write the code for releasing Virtual channel allocation */
		...
		if( pkt->data.pkttype == (pkt->data.packetsize - 1)/FLITSZ ) {
			...
			do {
				if( buf->vc_data.input_seqno[k] == pkt->data.seqno ) {
					...
					/*if( pkt->data.srccpu == 3 )
					if( (buf->id == 149) || (buf->id == 171) )
					printf("Releasing the virtual channel %d seqno %d\n", buf->id, pkt->data.seqno );*/

					newbuf->output_vc = -1;
					newbuf->output_port = -1;
					newbuf->output_seqno = -1;
					newbuf->output_credit = -1;
					newbuf->output_alloc = -1;

					buf->vc_data.input_vc[k] = -1;
					buf->vc_data.input_port[k] = -1;
					buf->vc_data.input_seqno[k] = -1;
					*((buf->vc_data.inputbuf)+k) = NULL;

					SemaphoreSignal(buf->bufva_sema);
					SemaphoreSet(buf->bufva_sema);
					break;
				}
				else
					k++;
...
/**************************************************************************************/
static int YS__Congestion(ibuf, obuf, pkt)
PACKET *pkt;
BUFFER *ibuf;
BUFFER *obuf;
{
	...
	if( ibuf->output_credit > 1 ) {
		if( SemaphoreValue(ibuf->bufcredit_sema) <= 0 ) {
...
/**************************************************************************************/
static void YS__DynamicBuffer(ibuf)
BUFFER *ibuf;
{
	DEMUX *newdemux;
	BUFFER *obuf, *buf;
	...
	if( ibuf->buftype == INPUT_BUFFER ) {
		newdemux = (DEMUX*)ibuf->prevmodule;

		if( newdemux->prevmodule != NULL ) {
			sum = 0;
			for( k = 0; k < VC; k++ ) {
				buf = (BUFFER*)*((newdemux->nextmodule)+k);
				sum = sum + (IBUFSZ - (buf->free));
			}

			obuf = (BUFFER*)newdemux->prevmodule;
			if( (BUFSZTOTAL-FLITSZ) < sum ) {
				if( SemaphoreValue(obuf->bufcc_sema) > 0 ) {
					SemaphoreDecr(obuf->bufcc_sema);
					obuf->vc_data.congestion = obuf->vc_data.congestion + 1;
				}
			}
			else {
				if( SemaphoreValue(obuf->bufcc_sema) <= 0 ) {
					SemaphoreSignal(obuf->bufcc_sema);
					SemaphoreSet(obuf->bufcc_sema);
				}
			}
		}
	}
...
/**************************************************************************************/
static void YS__StaticBuffer(ibuf)
BUFFER *ibuf;
{
	DEMUX *newdemux;
	BUFFER *obuf, *buf;
	...
	if( ibuf->buftype == INPUT_BUFFER ) {
		...
		if( newdemux->prevmodule != NULL ) {
			...
			for( k = 0; k < VC; k++ ) {
				buf = (BUFFER*)*((newdemux->nextmodule)+k);
				if( buf->output_seqno != -1 ) {
					if( buf->free <= bufthresh2 )
						sum++;
				}
			}
...
/**************************************************************************************/
void static YS__IBVirtualChannelRelease(inbuf, pkt)
BUFFER *inbuf;
PACKET *pkt;
{
...
/**************************************************************************************/
void static YS__IBCreditFlowControl(buf, pkt)
BUFFER *buf;
PACKET *pkt;
{
	...
	/*if( buf->buftype == INPUT_BUFFER ) {
		...
		if( newdemux->prevmodule != NULL ) {
			...
			prev_vcindex = ((buf->id)%(((DEGREE+1)*VC)+DEGREE+1))%(VC);*/

			sprintf(namestr,"pktcr%d",pkt->data.seqno);
			creditevent = NewEvent(namestr,YS__CreditEvent,DELETE,0);
      		ActivitySetArg(creditevent,buf,0);
      		ActivitySchedTime(creditevent,CREDIT_LOOP_DELAY,INDEPENDENT);
	...
	if( buf->buftype == INPUT_BUFFER ) {
		newdemux = (DEMUX*)buf->prevmodule;
   		if( newdemux->prevmodule != NULL ) {
   			...
   			/*printf("op buf %d Vcindex %d credit %d time %g\n",
   				output_prevbuf->id, prev_vcindex,
   				output_prevbuf->vc_data.input_credit[prev_vcindex], YS__Simtime);*/
...
int type;
{
	...
	if( buf->buftype == OUTPUT_BUFFER ) {
		...
		vcfree = 0;
		for( k = 0; k < VC; k++ ) {
			sum = sum + buf->vc_data.input_credit[k];
			if( buf->vc_data.input_vc[k] == -1 )
				vcfree++;
		}
